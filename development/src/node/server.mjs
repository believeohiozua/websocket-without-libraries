...
const server = createServer((request, response) => {
  ...
  .listen(5000, () => console.log('server listening to', PORT));
...
// console.log('head', socket)
    ...
    socket.handleUpgrade(request, socket, head, function done(ws) {
...
function onSocketReadable(socket) {
  ...
  if (lengthIndicatorInBits <= SEVEN_BITS_INTEGER_MARKER) {
    ...
    // messageLength = socket.read(8).readUint16BE(0)
  }
...
//send the messages to other listeners on the connection
